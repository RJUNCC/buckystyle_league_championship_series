...
class BLCSXStatsCog(commands.Cog):
    ...
    async def stat_leaders_command(self, ctx, stat: str, limit: int = 5):
        ...
        # Define choices for the stat parameter
        valid_stats = {
        ...
        if stat not in valid_stats:
            await ctx.response.send_message(f"Invalid stat. Choose from: {', '.join(valid_stats.keys())}", ephemeral=True)
            return
        ...
        try:
            ...
            stat_display_name = valid_stats[stat]
...
def setup(bot):
    bot.add_cog(BLCSXStatsCog(bot))# cogs/blcsx_stats.py
import discord
from discord.ext import commands
import aiohttp
import asyncio
